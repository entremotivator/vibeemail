...
st.sidebar.markdown("---")
st.sidebar.markdown('''<p class="section-label">📬 PostGrid — Send Physical Mail</p>''', unsafe_allow_html=True)

postgrid_api_key = st.sidebar.text_input(
    "PostGrid API Key",
    type="password",
    value=os.getenv("POSTGRID_API_KEY", ""),
    placeholder="test_sk_... or live_sk_...",
    label_visibility="collapsed",
    help="Use a test_sk_ key to preview without sending real mail."
)

postgrid_mail_class = st.sidebar.selectbox(
    "Mail Class",
    ["first_class", "standard_class", "express"],
    format_func=lambda x: {"first_class": "First Class (3–5 days)", "standard_class": "Standard Class (5–10 days)", "express": "Express (1–2 days)"}.get(x, x),
    key="postgrid_mail_class"
)

postgrid_certified = st.sidebar.checkbox(
    "Certified Mail + Return Receipt",
    value=True,
    help="Strongly recommended for legal letters — provides USPS proof of delivery."
)

postgrid_color = st.sidebar.checkbox("Color printing", value=False)
postgrid_double_sided = st.sidebar.checkbox("Double-sided printing", value=False)

st.sidebar.markdown("""
<div style="color: #555e72; font-size: 0.75rem; line-height: 1.5; margin-top: 4px;">
<b style="color:#c9a96e;">Test key</b> → previews only, no real mail sent.<br>
<b style="color:#c9a96e;">Live key</b> → physically mailed within 2 business days.<br>
<a href="https://dashboard.postgrid.com" target="_blank" style="color:#c9a96e;">Get API key at PostGrid →</a>
</div>
""", unsafe_allow_html=True)
...
# ─────────────────────────────────────────
# POSTGRID SEND FUNCTION
# ─────────────────────────────────────────
def send_via_postgrid(html_body: str, pdf_bytes: bytes) -> dict:
    """
    Send the letter via PostGrid Print & Mail API.
    Uses inline HTML (cleaner than uploading PDF for dispute letters).
    Returns the full API response dict.
    """
    POSTGRID_BASE = "https://api.postgrid.com/print-mail/v1"

    if not postgrid_api_key:
        raise ValueError("PostGrid API key is required. Add it in the sidebar.")
    ...
    # Parse recipient address — split on comma or newline
    recipient_lines = [l.strip() for l in company_address.replace(",", "\n").split("\n") if l.strip()]
    recipient_line1 = recipient_lines[0] if len(recipient_lines) > 0 else ""
    recipient_line2 = recipient_lines[1] if len(recipient_lines) > 1 else ""
    # city/state/zip from remaining lines
    recipient_city_state_zip = recipient_lines[-1] if len(recipient_lines) > 2 else recipient_lines[-1] if recipient_lines else ""

    # Parse city, state, zip from "City, ST 00000"
    def parse_csz(csz_str):
        csz_str = csz_str.strip()
        parts = csz_str.rsplit(" ", 1)
        postal_code = parts[-1].strip() if len(parts) > 1 else ""
        rest = parts[0].strip() if len(parts) > 1 else csz_str
        rest_parts = rest.rsplit(",", 1)
        city = rest_parts[0].strip() if len(rest_parts) > 1 else rest.strip()
        state = rest_parts[-1].strip() if len(rest_parts) > 1 else "NY"
        return city, state, postal_code

    to_city, to_state, to_zip = parse_csz(recipient_city_state_zip)
    from_city, from_state, from_zip = parse_csz(city_state_zip)

    # Build the HTML for PostGrid — needs to be a complete, self-contained HTML doc
    # PostGrid accepts HTML directly in the "html" field
    letter_html = html_body  # already a full HTML document from build_html()

    payload = {
        "to": {
            "name": f"{recipient_attn or company_name}",
            "companyName": company_name,
            "addressLine1": recipient_line1 or company_address.split("\n")[0],
            "addressLine2": recipient_line2 if recipient_line2 and recipient_line2 != recipient_city_state_zip else "",
            "city": to_city,
        ...
        "html": letter_html,
        "mailingClass": postgrid_mail_class,
        "doubleSided": postgrid_double_sided,
        "color": postgrid_color,
        "metadata": {
            "letter_type": letter_type,
            "account_name": account_name,
            "generated_date": today_date,
            "source": "credit_repair_letter_system",
        }
    }

    # Add certified mail if checked (PostGrid uses "express" or certified flag)
    if postgrid_certified:
        payload["mailingClass"] = "first_class"
        payload["certified"] = True

    response = requests.post(
        f"{POSTGRID_BASE}/letters",
        headers=headers,
        json=payload,
        timeout=30
    )
    ...
    if response.status_code not in (200, 201):
        ...
        raise RuntimeError(f"PostGrid API error ({response.status_code}): {error_msg}")
    ...
    if errors:
        ...
        with st.spinner("Drafting your letter..."):
            try:
                ...
                tab1, tab2, tab3, tab4 = st.tabs(["📄 Preview", "💾 Download", "🔤 Raw HTML", "📬 Send via PostGrid"])
                ...
                with tab4:
                    st.markdown('''<p class="section-label">Send This Letter via Physical Mail</p>''', unsafe_allow_html=True)

                    if not postgrid_api_key:
                        st.markdown("""
                        <div class="info-card">
                            <b style="color: #f0ede8;">PostGrid API Key Required</b><br>
                            Add your PostGrid API key in the sidebar to enable physical mail sending.
                            Dispute letters sent via Certified Mail have a <b>much higher response rate</b> than those sent digitally.<br><br>
                            <a href="https://dashboard.postgrid.com" target="_blank" style="color: #c9a96e;">
                            → Sign up for free at PostGrid (includes test credits)
                            </a>
                        </div>
                        """, unsafe_allow_html=True)
                    else:
                        # Show preview of mailing details
                        is_test = postgrid_api_key.startswith("test_")
                        mode_color = "#f5a623" if is_test else "#6ee89a"
                        mode_label = "TEST MODE — No real mail will be sent" if is_test else "LIVE MODE — Real mail will be physically sent"

                        st.markdown(f"""
                        <div class="info-card">
                            <b style="color: {mode_color};">⚠ {mode_label}</b><br><br>
                            <b style="color: #f0ede8;">To:</b> {company_name}<br>
                            &nbsp;&nbsp;&nbsp;&nbsp;{company_address.replace(chr(10), "<br>&nbsp;&nbsp;&nbsp;&nbsp;")}<br><br>
                            <b style="color: #f0ede8;">From:</b> {full_name}<br>
                            &nbsp;&nbsp;&nbsp;&nbsp;{address}, {city_state_zip}<br><br>
                            <b style="color: #f0ede8;">Mail Class:</b> {postgrid_mail_class.replace("_", " ").title()}<br>
                            <b style="color: #f0ede8;">Certified Mail:</b> {"Yes ✓" if postgrid_certified else "No"}<br>
                            <b style="color: #f0ede8;">Color Printing:</b> {"Yes" if postgrid_color else "No (B&W)"}<br>
                            <b style="color: #f0ede8;">Double-sided:</b> {"Yes" if postgrid_double_sided else "No"}
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        with pg_col1:
                            confirm_send = st.checkbox(
                                "I confirm this letter is correct and I want to send it via PostGrid.",
                                key="confirm_postgrid_send"
                            )
                        with pg_col2:
                            send_clicked = st.button(
                                ...
                                use_container_width=True,
                                key="send_postgrid_btn"
                            )

                        if send_clicked and confirm_send:
                            with st.spinner("Submitting to PostGrid..."):
                                try:
                                    pg_result = send_via_postgrid(final_html, generate_pdf(body_content).read())

                                    letter_id = pg_result.get("id", "N/A")
                                    pg_status = pg_result.get("status", "unknown")
                                    tracking_url = pg_result.get("url", "")
                                    expected_delivery = pg_result.get("expectedDeliveryDate", "")

                                    st.success(f"✅ Letter submitted to PostGrid successfully!")

                                    st.markdown(f"""
                                    <div class="info-card">
                                        <b style="color: #f0ede8;">📦 Submission Confirmed</b><br><br>
                                        <b>Letter ID:</b> <code style="color:#c9a96e;">{letter_id}</code><br>
                                        <b>Status:</b> {pg_status.title()}<br>
                                        {"<b>Expected Delivery:</b> " + expected_delivery + "<br>" if expected_delivery else ""}
                                        {"<b>Track:</b> <a href=\"" + tracking_url + "\" target=\"_blank\" style=\"color:#c9a96e;\">View in PostGrid Dashboard →</a><br>" if tracking_url else ""}
                                        <br>
                                        <b style="color: #f0ede8;">Next Steps:</b><br>
                                        1. Save your Letter ID for records: <code style="color:#c9a96e;">{letter_id}</code><br>
                                        2. Track delivery in your <a href="https://dashboard.postgrid.com" target="_blank" style="color:#c9a96e;">PostGrid Dashboard</a><br>
                                        3. Bureaus have <b>30 days</b> to respond after receiving your letter<br>
                                        4. Follow up if no response within 35 days
                                    </div>
                                    """, unsafe_allow_html=True)

                                    # Show full API response for reference
                                    with st.expander("🔍 Full API Response (for your records)"):
                                        st.json(pg_result)

                                except RuntimeError as pg_err:
                                    st.error(f"PostGrid send failed: {pg_err}")
                                    st.info("Check that your API key is valid and your PostGrid account has sufficient credits.")
                                except Exception as pg_err:
                                    st.error(f"Unexpected error: {pg_err}")
