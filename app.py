...
# ── Load buvie_frame_transparent.png ─────────────────────────────
...
<div class="shell">
  ...
  <header>
    <h1>✦ &nbsp;BUVIE WEEKEND BIRTHDAY&nbsp; ✦</h1>
    <div class="sub">Miami Celebration &nbsp;·&nbsp; Photo Booth Experience</div>
  </header>
  ...
  <div class="panels">
    ...
    <div class="panel active" id="panelCam">
      ...
      <div class="btnrow" id="camAfter" hidden>
        <button class="btn gold"    onclick="downloadCanvas(snapC,'buvie_weekend_birthday.png')">⬇ Download PNG</button>
    ...
    <div class="panel" id="panelUp">
      ...
      <div class="btnrow" id="upAfter" hidden>
        <button class="btn gold"    onclick="downloadCanvas(upC,'buvie_weekend_birthday.png')">⬇ Download PNG</button>
...
async function startCamera(f){{
  ...
  if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){{
    camSt.textContent='⚠ Camera not available — try opening app directly in browser';
    return;
  }}
  const tries=[
...
function renderGal(){{
  ...
  gallery.forEach((url,i)=>{{
    ...
    btn.onclick=()=>{{
      const a=document.createElement('a');
      a.href=url; a.download=`buvie_weekend_${{i+1}}.png`;
...
if not FRAME_OK:
    st.error(
        "⚠️ **buvie_frame_transparent.png not found.**  "
        "Place your birthday frame image as `buvie_frame_transparent.png` in the same folder as `app.py`, then refresh.",
        icon="🖼",
    )
else:
    ...
    (function(){
      ...
      patch();
      new window.parent.MutationObserver(patch).observe(
