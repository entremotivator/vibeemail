...
import io, os, base64, datetime, json, time
...
[data-testid="stMainBlockContainer"],section[data-testid="stMain"]{
  background:#060610!important;overflow:hidden!important;height:100vh!important;max-height:100vh!important;
}
...
# Measured cutout in the 1086×1448 frame image
...
def get_or_create_folder():
    ...
    try:
        ...
        # Check if folder already exists today
...
def upload_to_drive(img_bytes: bytes, filename: str, folder_id: str):
    """Upload PNG bytes → Drive. Returns (file_id, view_url, thumb_url) or (None,None,None)."""
    try:
        from googleapiclient.http import MediaIoBaseUpload
        svc = get_drive_service()
        if not svc or not folder_id: return None, None, None
        ...
        thumb = f"https://drive.google.com/thumbnail?id={fid}&sz=w600"
        dl    = f"https://drive.google.com/uc?export=download&id={fid}"
        return fid, view, thumb, dl
    except Exception as e:
        return None, None, None, None

def list_drive_photos(folder_id: str):
    """Return list of dicts {name, thumb, view, dl} for all PNGs in the folder."""
    try:
        svc = get_drive_service()
        if not svc or not folder_id: return []
        q = f"'{folder_id}' in parents and mimeType='image/png' and trashed=false"
        files = svc.files().list(q=q, fields="files(id,name)", orderBy="createdTime desc", pageSize=50).execute().get("files",[])
        out = []
        for f in files:
            fid = f["id"]
            out.append({
...
if "gallery" not in st.session_state:
    # Load existing Drive photos on first load
    st.session_state.gallery = list_drive_photos(st.session_state.folder_id)
if "upload_pending" not in st.session_state:
    st.session_state.upload_pending = None   # bytes when JS posts a photo

# ── Handle upload submitted via query-param trick ──────────────
qp = st.query_params
if "img_b64" in qp and "img_name" in qp:
    try:
        img_bytes = base64.b64decode(qp["img_b64"])
        fname     = qp["img_name"]
        fid, view, thumb, dl = upload_to_drive(img_bytes, fname, st.session_state.folder_id)
        if fid:
...
HOST_EMAIL   = st.secrets.get("app", {}).get("host_email", "")
SHARE_EMAIL  = st.secrets.get("app", {}).get("share_email", HOST_EMAIL)
...
:root{{
  ...
  --border:rgba(212,175,55,.18);--borderact:rgba(212,175,55,.5);
...
header{{
  ...
  background:rgba(6,6,16,.97);border-bottom:1px solid var(--border);
  flex-shrink:0;
}}
header h1{{
  font-size:clamp(1rem,4vw,1.5rem);font-weight:900;
  letter-spacing:.16em;color:var(--gold3);
  text-shadow:0 0 28px rgba(255,245,200,.25);line-height:1.1;
  font-family:'Georgia',serif;
}}
header .sub{{
  font-size:clamp(.65rem,2vw,.8rem);color:rgba(212,175,55,.45);
  letter-spacing:.1em;margin-top:2px;font-style:italic;
}}
.drive-pill{{
  ...
  transition:all .2s;cursor:pointer;background:rgba(212,175,55,.04);
}}
.drive-pill:hover{{border-color:var(--gold);color:var(--gold2);}}
.drive-dot{{width:6px;height:6px;border-radius:50%;background:#4ade80;
  box-shadow:0 0 5px rgba(74,222,128,.5);animation:pulse 2s infinite;}}
...
.tab{{
  ...
  color:rgba(212,175,55,.35);cursor:pointer;
...
.badge{{
  ...
  background:var(--gold);color:#060610;
...
.panel{{
  ...
  opacity:0;pointer-events:none;transition:opacity .18s;overflow:hidden;
}}
...
.ph{{
  color:rgba(212,175,55,.35);font-size:.85rem;
  text-align:center;padding:20px;line-height:1.8;font-style:italic;
}}
...
.stbar{{
  ...
  color:rgba(212,175,55,.5);font-style:italic;flex-shrink:0;
}}
...
.btn{{
  flex:1;padding:11px 6px;border:none;border-radius:7px;cursor:pointer;
...
.btn.gold{{
  background:linear-gradient(135deg,#5a4010,#c9a62e,#7a5c18,#e0bc4a,#6a4c10);
  color:#060610;box-shadow:0 4px 16px rgba(212,175,55,.35);flex:2;font-size:.75rem;
}}
.btn.outline{{
  background:rgba(212,175,55,.07);
  border:1px solid rgba(212,175,55,.35);color:rgba(212,175,55,.75);
}}
...
/* ── DROP ZONE ── */
.drop{{
  border:2px dashed rgba(212,175,55,.25);border-radius:8px;
  padding:18px 12px;text-align:center;cursor:pointer;
  background:var(--bg2);transition:all .2s;flex-shrink:0;
}}
...
/* ── UPLOAD SPINNER ── */
#uploadOverlay{{
  position:absolute;inset:0;background:rgba(6,6,16,.88);
  display:none;flex-direction:column;align-items:center;justify-content:center;
  gap:12px;z-index:50;border-radius:8px;
}}
...
.spin-txt{{font-size:.78rem;color:rgba(212,175,55,.65);font-style:italic;letter-spacing:.08em;}}
...
.gal-item{{position:relative;border-radius:7px;overflow:hidden;
...
.gal-item-acts{{
  ...
  background:linear-gradient(transparent,rgba(6,6,16,.93));
  padding:20px 5px 5px;display:flex;gap:4px;
  opacity:0;transition:opacity .2s;
}}
.gal-item:hover .gal-item-acts,
.gal-item:active .gal-item-acts{{opacity:1;}}
.gab{{
  flex:1;padding:6px 3px;
  background:rgba(212,175,55,.15);border:1px solid rgba(212,175,55,.3);
  border-radius:5px;color:var(--gold2);cursor:pointer;
  font-weight:700;font-size:.52rem;letter-spacing:.07em;text-transform:uppercase;
}}
.gal-empty{{
  text-align:center;color:rgba(212,175,55,.3);font-style:italic;
  font-size:.88rem;margin:40px auto;line-height:1.9;
}}
...
#sheetBody{{
  position:relative;background:var(--bg2);border-radius:16px 16px 0 0;
  border:1px solid var(--border);
  padding:16px 14px calc(16px + var(--safe));
  display:flex;flex-direction:column;gap:9px;
}}
.sh-title{{text-align:center;font-size:.7rem;letter-spacing:.18em;text-transform:uppercase;
  color:var(--gold2);padding-bottom:9px;border-bottom:1px solid rgba(212,175,55,.1);}}
.sh-btn{{
...
/* ── TOAST ── */
#toast{{
  position:fixed;bottom:calc(20px + var(--safe));left:50%;
  transform:translateX(-50%) translateY(16px);
  background:rgba(16,16,42,.97);border:1px solid var(--gold);
  color:var(--gold2);padding:9px 20px;border-radius:100px;
  font-size:.66rem;letter-spacing:.12em;font-weight:700;text-transform:uppercase;
  opacity:0;transition:all .28s;z-index:999;white-space:nowrap;pointer-events:none;
}}
#toast.show{{opacity:1;transform:translateX(-50%) translateY(0);}}

/* ── FOOTER ── */
footer{{
  text-align:center;padding:4px 16px calc(4px + var(--safe));
  font-size:.6rem;color:rgba(212,175,55,.18);letter-spacing:.06em;font-style:italic;
  border-top:1px solid var(--border);background:rgba(6,6,16,.97);flex-shrink:0;
}}
</style>
</head>
<body>
...
<div class="shell">
  <header>
    ...
    {'<a class="drive-pill" href="' + FOLDER_URL + '" target="_blank"><span class="drive-dot"></span>Drive folder ready · tap to view</a>' if FOLDER_URL else '<div class="drive-pill"><span class="drive-dot off"></span>Drive not connected</div>'}
  </header>
  ...
  <div class="panels">
    ...
    <div class="panel active" id="panelCam">
      <div class="cvs-wrap">
        ...
        <div class="ph" id="camPH"><span>📷</span>Starting camera…<br><small style="font-size:.7rem;opacity:.6">Allow camera access when prompted</small></div>
        <canvas id="liveC" style="display:none"></canvas>
        <canvas id="snapC" style="display:none"></canvas>
      </div>
      <div class="stbar"><span class="dot" id="camDot"></span><span id="camSt">Requesting camera…</span></div>
    ...
    <div class="panel" id="panelUp">
      ...
      <input type="file" id="fi" accept="image/*" capture="environment" style="display:none">
      <div class="cvs-wrap" id="upWrap" style="display:none">
        <canvas id="upC"></canvas>
      </div>
      <div class="stbar" id="upStatus" style="display:none">
...
<script>
// ── CONFIG ────────────────────────────────────────────────────
const DRIVE_OK   = {DRIVE_OK};
const FOLDER_URL = "{FOLDER_URL}";
const SHARE_EMAIL= "{SHARE_EMAIL}";

// Seed gallery from Drive (server-side rendered)
...
(function(){{
  ...
  for(let i=0;i<100;i++){{
    ...
    d.style.cssText=`position:absolute;width:${{sz}}px;height:${{sz}}px;`
      +`background:#fff8e0;border-radius:${{Math.random()>.5?'50%':'2px'}};`
...
document.querySelectorAll('.tab').forEach(t=>{{
  t.addEventListener('click',()=>{{
    ...
    const map={{cam:'panelCam',up:'panelUp',gal:'panelGal'}};
...
async function startCamera(f){{
  ...
  stream=null;
  setStatus('Starting '+(f==='environment'?'rear':'front')+' camera…','');
  if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){{
    setStatus('⚠ Camera API blocked — open app directly in browser','warn');
    document.getElementById('camPH').innerHTML='<span>⚠</span>Camera blocked<br><small style="font-size:.7rem;opacity:.6">Open app in your phone browser directly</small>';
    return;
  }}
  const tries=[
  ...
  for(const c of tries){{
    try{{ stream=await navigator.mediaDevices.getUserMedia(c); break; }}
      ...
      if(e.name==='NotAllowedError'||e.name==='PermissionDeniedError'){{
        setStatus('⚠ Camera denied — allow in browser settings','warn'); return;
      }}
    }}
  }}
  if(!stream){{ setStatus('⚠ Camera unavailable','warn'); return; }}
  ...
  reader.onload=ev=>{{
    ...
    img.onload=()=>{{
      function go(){{
        ...
        document.getElementById('upSt').textContent='✅ Framed — tap Share!';
        toast('✦ Frame applied!');
      }}
...
function updateBadge(){{
  ...
  const c=document.getElementById('galCount');
...
function addToGallery(canvas, fname){{
  canvas.toBlob(blob=>{{
    const url=URL.createObjectURL(blob);
    gallery.unshift({{name:fname,thumb:url,view:url,dl:url,local:true}});
    updateBadge(); renderGal(); toast('✦ Added to gallery!');
  }},'image/png');
}}
...
function renderGal(){{
  ...
  gallery.forEach((item,i)=>{{
    ...
    dlBtn.onclick=e=>{{
      e.stopPropagation();
      if(item.local){{
        const a=document.createElement('a');a.href=item.dl;a.download=item.name||'photo.png';
        document.body.appendChild(a);a.click();document.body.removeChild(a);
      }}else window.open(item.dl,'_blank');
      toast('✦ Saved!');
    }};
    // View on Drive button (non-local only)
    if(!item.local&&item.view){{
      const vBtn=document.createElement('button'); vBtn.className='gab'; vBtn.textContent='☁ View';
      vBtn.onclick=e=>{{e.stopPropagation();window.open(item.view,'_blank');}};
      acts.appendChild(vBtn);
    }}
    acts.appendChild(dlBtn);
    wrap.appendChild(img); wrap.appendChild(acts); grid.appendChild(wrap);
  }});
  updateBadge();
}}
...
async function uploadToDrive(canvas, fname){{
  ...
  showUploadOverlay('Saving to Drive…');
  try{{
    const blob = await new Promise(res=>canvas.toBlob(res,'image/png'));
    const b64full = await blobToB64(blob);
    const b64 = b64full.split(',')[1];
    // Post to Python backend via URL query params → triggers rerun → upload → gallery update
    const url = new URL(window.location.href);
    url.searchParams.set('img_b64', b64);
    url.searchParams.set('img_name', fname);
...
function showUploadOverlay(msg){{
  ...
  document.getElementById('uploadTxt').textContent=msg||'Saving…';
...
function emailPhoto(canvas,fname){{
  ...
  setTimeout(()=>{{
    ...
    const bdy=encodeURIComponent(
      'Here\'s a photo from Buvie Weekend! 🎉\n\n'
...
function boot(){{
  ...
  renderGal();
  updateBadge();
  // Patch parent iframe for camera permission
  try{{
...
if not FRAME_OK:
    ...
    # Patch iframes in parent Streamlit page to allow camera
    ...
    (function(){
      ...
      patch();
      try{new MutationObserver(patch).observe(document.body,{childList:true,subtree:true});}catch(e){}
    })();
    </script>""", height=0)
